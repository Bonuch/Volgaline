<template>
    <div class="page-inner page-river-cruises-2022 pt-6">
        <div class="wrapper">
            <Breadcrumbs v-if="breadcrumbs" :breadcrumbs="breadcrumbs" class="mb-14"/>
        </div>

        <div class="page-inner__content">
            <section class="page-river-cruises-2022-heading mb-20 pb-20 sm:pb-6 sm:mb-14">
                <div class="wrapper wrapper-1410">
                    <h1 class="page-inner-h1 page-inner-h1--big mb-13 sm:mb-16">
                        Речные круизы 2022 года из Москвы на&nbsp;теплоходах - расписание и&nbsp;цены на&nbsp;речные
                        круизы
                    </h1>

                    <p class="m-0 text-lg leading-1-9 mob:text-base">
                        Круизы из Москвы на теплоходе — самый романтичный и необычный способ путешествовать по России.
                        Наша компания разработала интересные маршруты, охватывающие крупные города страны и небольшие,
                        но не менее живописные населенные пункты. Организуем речные прогулки по Волге, Оке, Енисею
                        на современных комфортабельных суднах. В стоимость круиза включены как минимум одна экскурсия,
                        трехразовое питание и обширная программа развлечений.
                    </p>
                </div>
            </section>

            <section class="mb-14">
                <div class="wrapper wrapper-1410">
                    <h2>Популярные круизные направления из Москвы</h2>

                    <PopularDirectionBanners
                        v-if="directions && directions.length > 0"
                        :arr="directions"
                    />

                    <p v-else class="m-0">Направления отсутствуют</p>
                </div>
            </section>

            <section class="mb-20 py-14 bg-gray-50 sm:mb-14 sm:pb-12">
                <div class="wrapper wrapper-1410">
                    <h2>Популярные теплоходы</h2>

                    <PopularDirectionBanners
                        v-if="ships && ships.length > 0"
                        :arr="ships"
                    >
                        <template v-slot:link="{ item }">
                            <nuxt-link
                                :to="`/populyarnye-kruiznye-teplohody/${item.slug}`"
                                :title="item.title"
                                class="m-0 text-26 font-bold leading-1-1 line-clamp-2 h-auto"
                            >
                                Т/х «{{ item.title }}»
                            </nuxt-link>
                        </template>
                    </PopularDirectionBanners>

                    <p v-else class="m-0">Теплоходы заполнены</p>
                </div>
            </section>

            <section class="cruises-city cruises-city-3">
                <div class="wrapper wrapper-1410">
                    <div class="cruises-city-wrap">
                        <h2>Круизы по городам России из Москвы</h2>

                        <div class="cruises-city-list">
                            <div
                                v-for="(item, itemIdx) in cruise_city"
                                :key="itemIdx"
                                class="cruises-city-item pb-16 last:pb-0 tablet:pb-4"
                            >
                                <strong
                                    v-if="item.title"
                                    class="cruises-city-title--square"
                                >
                                    {{ item.title }}
                                </strong>
                                <template v-if="item.items && item.items.length > 0">
                                    <nuxt-link
                                        v-for="(subItem, subIdx) in item.items"
                                        :key="subIdx"
                                        class="cruises-city-link block pb-4 text-lg"
                                        :title="subItem.title"
                                        :to="'/'+subItem.link"
                                    >
                                        {{ subItem.title }}
                                    </nuxt-link>
                                </template>
                            </div>
                        </div>
                    </div>
                </div>
            </section>

            <section v-if="cruiseDurationBanners && cruiseDurationBanners.length > 0"
                     class="mb-20 bg-gray-50 pt-16 pb-10 tablet:pt-10 tablet:mb-10">
                <div class="wrapper wrapper-1410 overflow-x-hidden">
                    <h2>Выбрать продолжительность круиза</h2>

                    <template v-if="isMobileScreen">
                        <client-only>
                            <BaseMobileSlider
                                :buttons="true"
                                :pagination="true"
                            >
                                <CruiseBannerCard
                                    v-for="(item) in cruiseDurationBanners"
                                    :key="item.id"
                                    :item="item"
                                    class="swiper-slide"
                                />
                            </BaseMobileSlider>
                        </client-only>
                    </template>

                    <div
                        v-else
                        class="flex flex-wrap overflow-x-hidden -mx-4 tablet:mx-0"
                    >
                        <CruiseBannerCard
                            v-for="(item) in cruiseDurationBanners"
                            :key="item.id"
                            :item="item"
                            class="w-1/3 px-4 mb-10 xl:w-1/2 tablet:w-full tablet:mb-4 tablet:last:mb-0 tablet:px-0"
                        />
                    </div>
                </div>
            </section>

            <section v-if="seasonCruises && seasonCruises.length > 0" class="mb-10">
                <div class="wrapper wrapper-1410 overflow-x-hidden">
                    <h2>Сезонные круизы</h2>

                    <template v-if="isMobileScreen">
                        <client-only>
                            <BaseMobileSlider
                                :buttons="true"
                                :pagination="true"
                            >
                                <CruiseBannerCard
                                    v-for="(item) in seasonCruises"
                                    :key="item.id"
                                    :item="item"
                                    class="swiper-slide"
                                />
                            </BaseMobileSlider>
                        </client-only>
                    </template>

                    <div
                        v-else
                        class="flex flex-wrap overflow-x-hidden -mx-4 tablet:mx-0"
                    >
                        <CruiseBannerCard
                            v-for="(item) in seasonCruises"
                            :key="item.id"
                            :item="item"
                            class="w-1/3 px-4 mb-10 xl:w-1/2 tablet:w-full tablet:mb-4 tablet:last:mb-0 tablet:px-0"
                        />
                    </div>
                </div>
            </section>

            <div class="page-river-cruises-2022-search wrapper wrapper-1410">
                <h2>Соберите свой круиз сами</h2>

                <div class="relative z-1 p-4 -mx-4 bg-blue-900 rounded-2xl hidden xl:block">
                    <button
                        type="button"
                        class="flex items-center justify-center w-full p-3 bg-white text-black-400 text-center text-base font-medium rounded-sm custom-transition hover:text-blue-400"
                        @click="openModal({ name: 'ModalFilter' })"
                    >
                        <svg
                            class="mr-1 fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                        >
                            <path
                                class="fill-current" fill-rule="evenodd" clip-rule="evenodd"
                                d="M4.9375 2.81249C4.93749 2.2602 4.48977 1.81249 3.93749 1.8125C3.3852 1.81251 2.93749 2.26023 2.9375 2.81251L2.93756 8.5625H2.25C1.69772 8.5625 1.25 9.01022 1.25 9.5625C1.25 10.1148 1.69772 10.5625 2.25 10.5625H3.93758H5.625C6.17729 10.5625 6.625 10.1148 6.625 9.5625C6.625 9.01022 6.17729 8.5625 5.625 8.5625H4.93756L4.9375 2.81249ZM14.0626 1.8125C14.6149 1.8125 15.0626 2.26022 15.0626 2.81251L15.0625 10.8125H15.75C16.3023 10.8125 16.75 11.2602 16.75 11.8125C16.75 12.3648 16.3023 12.8125 15.75 12.8125H14.0625H12.375C11.8227 12.8125 11.375 12.3648 11.375 11.8125C11.375 11.2602 11.8227 10.8125 12.375 10.8125H13.0625L13.0626 2.81249C13.0626 2.26021 13.5103 1.8125 14.0626 1.8125ZM9.00002 1.8125C9.5523 1.8125 10 2.26022 10 2.8125L10 5.1875H10.6875C11.2398 5.1875 11.6875 5.63522 11.6875 6.1875C11.6875 6.73978 11.2398 7.1875 10.6875 7.1875H9H7.3125C6.76022 7.1875 6.3125 6.73978 6.3125 6.1875C6.3125 5.63522 6.76022 5.1875 7.3125 5.1875H8L8.00002 2.8125C8.00002 2.26021 8.44773 1.8125 9.00002 1.8125ZM9 7.4375C9.55228 7.4375 10 7.88521 10 8.4375L10 15.1875C10 15.7398 9.5523 16.1875 9.00002 16.1875C8.44773 16.1875 8.00002 15.7398 8.00002 15.1875L8 8.4375C8 7.88522 8.44771 7.4375 9 7.4375ZM15.0625 14.0624C15.0625 13.5101 14.6147 13.0625 14.0624 13.0625C13.5101 13.0625 13.0625 13.5103 13.0625 14.0626L13.0626 15.1876C13.0626 15.7399 13.5104 16.1875 14.0626 16.1875C14.6149 16.1875 15.0626 15.7397 15.0626 15.1874L15.0625 14.0624ZM3.9376 10.8125C4.48988 10.8125 4.93759 11.2602 4.93757 11.8125L4.9375 15.1875C4.93749 15.7398 4.48976 16.1875 3.93748 16.1875C3.38519 16.1875 2.93749 15.7398 2.9375 15.1875L2.93757 11.8125C2.93759 11.2602 3.38531 10.8125 3.9376 10.8125Z"
                                fill="white"
                            />
                        </svg>

                        <span class="link-inline">Показать фильтры</span>
                    </button>
                </div>

                <SearchComponent
                    v-if="isWideScreen"
                    class="mx-auto"
                    :expanded="true"
                    :show-expand-button="false"
                    @on-submit="handleSearchSubmit"
                />

                <div :class="{ 'disabled': fsrLoading }">
                    <CruiseSectionList
                        v-if="fsrContent && fsrContent.length > 0"
                        class="pt-20 bg-white xl:pt-10"
                        :cruises="fsrContent"
                        :sort-list="sortList"
                        :view-controls="viewControls"
                        :headings="headings"
                        :show-more-visible="fsrShowMoreVisibility"
                        @on-clear-query-params-click="onClearQueryParamsClick"
                        @on-show-more-click="fsrOnSearchMoreClick('/cruise/search')"
                    />

                    <div v-else class="wrapper mt-24 xl:pt-16">
                        <p class="p-2 m-0 mb-3 text-center font-medium">
                            Результаты отсутствуют, попробуйте задать другие параметры поиска.
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
import {mapGetters, mapActions, mapMutations} from 'vuex';
import {seoHelper, fetchSearchResult} from '@/mixins/content-helpers';
import {sfmPrepearFormData, sfmQueryHasProp} from '../../helpers/search';
import Breadcrumbs from '@/components/base/Breadcrumbs';
import PopularDirectionBanners from '@/components/inner/PopularDirectionBanners';
import SearchComponent from '@/components/search/SearchComponent';
import CruiseSectionList from '@/components/cruise-section/CruiseSectionList';
import BaseMobileSlider from '@/components/base/BaseMobileSlider';
import CruiseBannerCard from '@/components/front-banners/CruiseBannerCard';

const pageTitle = 'Речные круизы 2022 года из Москвы на теплоходах - расписание и цены на речные круизы';

export default {
    components: {
        CruiseBannerCard,
        BaseMobileSlider,
        CruiseSectionList,
        SearchComponent,
        PopularDirectionBanners,
        Breadcrumbs,
    },
    mixins: [seoHelper, fetchSearchResult],
    middleware({ redirect }) {
        redirect(301, "/");
    },
    head() {
        return {
            title: pageTitle,
            meta: [
                {
                    hid: 'keywords',
                    name: 'keywords',
                    content: this.getSeoField('keywords', 'Речные круизы, круизы на теплоходах, речная навигация, речное путешествие, речные туры, теплоходы, пароходы'),
                },
                {
                    hid: 'description',
                    name: 'description',
                    content: this.getSeoField('description', 'Речные круизы от компании «Волга Лайн» по всей России на речную навигацию 2021 и 2022 года. Действуют сезонные скидки до 20% на речные круизы 2022 года'),
                },
                {hid: 'og:title', property: 'og:title', content: this.getPageTitle()},
                {hid: 'og:type', property: 'og:type', content: this.getSeoType('website')},
                {
                    hid: 'og:description',
                    property: 'og:description',
                    content: this.getSeoField('description', 'Речные круизы от компании «Волга Лайн» по всей России на речную навигацию 2021 и 2022 года. Действуют сезонные скидки до 20% на речные круизы 2022 года'),
                },
                {hid: 'og:url', property: 'og:url', content: this.getPageUrl},
                {hid: 'og:image', property: 'og:image', content: this.getOgImage()},
            ],
        };
    },
    data() {
        return {
            cruise_city: [],
            sortList: [
                {
                    name: 'departure_date_from',
                    title: 'Дате начала',
                    value: 'asc',
                },
                {
                    name: 'price',
                    title: 'Стоимости',
                    value: 'asc',
                },
                {
                    name: 'duration',
                    title: 'Продолжительности',
                    value: 'asc',
                },
            ],
            viewControls: [
                {
                    name: 'grid',
                    icon: 'GridIcon',
                },
                {
                    name: 'list',
                    icon: 'ListIcon',
                },
                {
                    name: 'line',
                    icon: 'LineIcon',
                },
            ],
            headings: [
                'Дата',
                'Дней',
                'Название Круиза',
                'Маршрут',
                'Теплоход',
                'Кают',
                'Цена от',
                '',
            ],

            title: pageTitle,
            content: {},
            breadcrumbs: [
                {
                    id: 1,
                    title: 'Главная',
                    link: '/',
                },
                {
                    id: 3,
                    title: pageTitle,
                    link: '',
                },
            ],
            directions: [],
            cruiseDurationBanners: [],
            seasonCruises: [],
            ships: [],
            cruisesAndDirections: [],
        };
    },
    computed: {
        ...mapGetters(['getWindowWidth']),
        isWideScreen() {
            return this.getWindowWidth > 1280;
        },
        isMobileScreen() {
            return this.getWindowWidth < 1025;
        },
    },
    methods: {
        ...mapActions({
            openModal: 'modals/openModal',
            'fetchAllOptions': 'search/fetchAllOptions',
            'fetchSelectedOptions': 'search/fetchSelectedOptions',
        }),
        ...mapMutations({
            'setSelectedOptions': 'search/setSelectedOptions',
        }),
        async handleSearchSubmit(formData) {
            await this.fsrSearch("/cruise/search", formData);
        },
        async onClearQueryParamsClick() {
            const params = sfmPrepearFormData(this.$route.query, this.$dayjs);
            await this.fsrSearch("/cruise/search", params);
        },
    },
    async asyncData({$axios, query, $dayjs}) {
        let directions = [],
            ships = [],
            cruiseDurationBanners = [],
            seasonCruises = [],
            cruises = [],
            fsrContent = [],
            cruise_city = [],
            fsrContentMeta = {};

        try {
            const {data} = await $axios.get('/river-cruises-by-city');
            directions = data.directions;
            ships = data.ships;
            cruiseDurationBanners = data.cruise_duration_banners;
            seasonCruises = data.season_cruises;
        } catch (ex) {
            console.log('cant fetch page data: ' + ex);
        }

        try {
            const queryParams = sfmPrepearFormData(query, $dayjs);
            const {data} = await $axios.get('/cruise/search', {params: queryParams});
            fsrContent = data.data;
            fsrContentMeta = data.meta;
        } catch (ex) {
            console.log('cant fetch content: ' + ex);
        }
        try {
            let data = await $axios.get('/get-menu/cruise-menu');
            for (const dataKey in data.data) {
                let letter = data.data[dataKey].title;
                let obj = {title: letter};
                let items = [];

                for (const city in data.data[dataKey].children) {
                    let item = data.data[dataKey].children[city];
                    items.push({title: item.title, link: item.link})
                }
                obj.items = items;
                cruise_city.push(obj);
            }
        } catch (ex) {
            console.log('cant fetch cruises list: ' + ex);
        }

        return {
            directions,
            ships,
            cruise_city,
            cruiseDurationBanners,
            seasonCruises,
            cruises,
            fsrContent,
            fsrContentMeta,
        };
    },
    async fetch() {
        const params = sfmPrepearFormData(this.$route.query, this.$dayjs);
        await this.fetchAllOptions();

        if (params && Object.keys(params).length > 0 && sfmQueryHasProp(params)) {
            await this.fetchSelectedOptions(params);
        } else {
            this.setSelectedOptions(null);
        }
    },
    async beforeRouteUpdate(to, from, next) {
        const params = sfmPrepearFormData(to.query, this.$dayjs);
        if (params && Object.keys(params).length > 0) {
            // подгрузка параметров поиска в поисковую форму
            if (sfmQueryHasProp(params)) {
                await this.fetchSelectedOptions(params);
            }

            // подгрузка отсортированных круизов
            if (sfmQueryHasProp(params, ['sort', 'order']) || params['modal-filter']) {
                const params = sfmPrepearFormData(to.query, this.$dayjs);
                await this.fsrSearch('/cruise/search', params);
            }
        } else {
            this.setSelectedOptions(null);
        }

        next();
    },
};
</script>

<style lang="scss" scoped>
.page-river-cruises-2022 {
    &-heading {
        background: #ffffff url("~/assets/images/inner/waves.svg") no-repeat center bottom;
    }

    &::v-deep {
        .page-river-cruises-2022-search {
            .cruise-section-list .wrapper {
                padding: 0;
            }
        }
    }
}

.page-inner__content {
    h2 {
        @apply mb-8 text-44 text-black-900 xl:text-36 sm:text-28 mob:text-2xl;
    }
}

.cruises-city {
    h2 {
        margin-bottom: 70px;
    }
}

@screen sm {
    .page-river-cruises-2022 {
        &-heading {
            background-image: url("~/assets/images/inner/waves-mobile.png");
            background-size: 100% 100%;
            background-position: left bottom;
        }
    }

    .cruises-city {
        h2 {
            margin-bottom: 40px;
        }
    }
}

@screen sm {
    .page-river-cruises-2022 {
        &-heading {
            background-size: auto;
        }
    }
}
</style>
