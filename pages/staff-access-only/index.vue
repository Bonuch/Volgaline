    ...
    <div class="page-inner page-staff-access-only pt-6 pb-12">
        <div class="wrapper wrapper-1410">
            ...
            <form v-if="!isLogged"
                ...
                method="post"
                @submit.prevent="handleManagerLogin"
            >
            ...
            <div v-if="isLogged && !redirecting" class="mt-8">
                ...
                <p>
                    Перейти в <nuxt-link to="/rechnye-kruizy" class="underline">поиск круизов</nuxt-link>.
                </p>
            </div>
            <div v-if="isLogged && redirecting" class="">Успешная авторизация, перенаправляем на главную страницу...</div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        async handleManagerLogin() {
            ...
            try {
                const { data } = await this.$axios.post('/login-manager', this.managerForm);
                ...
                console.log('manager login error: ' + ex);
                this.loading = false;
