<template>
    <div class="discount-ships">
        <div class="swiper-container">
            <div class="swiper-wrapper">
                <div
                    v-for="(item) in arr"
                    :key="item.id"
                    class="swiper-slide discount-ships-slide overflow-hidden"
                >
                    <div class="absolute top-0 flex">
                        <div v-if="item.hot" class="excursion-card__decor">
                            <svg
                                class="discount-ships__icon"
                                width="18"
                                height="24"
                                viewBox="0 0 18 24"
                                fill="none"
                    ...
                    <div class="discount-ships-image">
                        <nuxt-link
                            ...
                            <div
                                :title="item.title"
                            >
                                <img :src="item.image" :alt="item.title" width="450" height="200" class="tablet:w-[calc(100%-60px)] rounded">
                            </div>
                        </nuxt-link>
                    </div>

                    <div class="discount-ships-content flex flex-col">
                        <p v-if="item.title" class="line-clamp-2 text-lg text-black-400 mb-1 font-medium sm:text-start">{{ item.title }}</p>

                        <nuxt-link
                            v-if="item.link && item.title"
                            :to="item.link"
                            class="font-normal bg-transparent text-red-500 text-left text-sm sm:text-start"
                            :title="item.title"
                        >
                            Подробнее
                        </nuxt-link>
