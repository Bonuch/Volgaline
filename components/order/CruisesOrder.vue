<template>
    <div class="order-total-item border-2 border-gray-50 rounded-sm overflow-hidden">
        <div class="relative pt-14 pb-6 px-5 bg-gray-50 sm:pb-9">
            <span
                class="order-total-item__badge inline-block absolute py-2 px-3 top-1 left-1 bg-blue-900 text-white text-sm leading-none text-center"
            >
               Круиз {{ count + 1 }}
            </span>
            ...
            <h3
                v-if="orderItem.name"
                class="m-0 mb-2 text-lg leading-1-1 text-blue-900 sm:mb-1 sm:text-base sm:leading-none"
            >
                <NuxtLink :to="`/rechnye-kruizy/${orderItem.cruiseSlug}`">{{ orderItem.name }}</NuxtLink>
            </h3>

            <div class="flex justify-start items-center mb-3 text-base sm:text-sm sm:mb-6">
                <div v-if="orderItem.departure_date_time">
                    {{ getMobileDateFormatted(orderItem.departure_date_time) }}
                </div>

                <div class="mx-2">&mdash;</div>

                <div v-if="orderItem.arrival_date_time" class="mr-5">
                    {{ getMobileDateFormatted(orderItem.arrival_date_time) }}
                </div>

                <div v-if="orderItem.days">
                    <span>{{ orderItem.days }}</span>
                    <span>{{ getDaysFormatted(orderItem.days) }}</span>
                </div>
            </div>

            <div v-if="orderItem.ship" class="flex items-center mb-3 text-gray-300 sm:mb-5">
                <svg
                    class="sm:w-6 sm:h-6"
                    width="32"
                    height="32"
                ...
                <span class="ml-3 text-lg font-medium sm:text-base">{{ orderItem.ship }}</span>
            </div>

            <div v-if="orderItem.road" class="font-medium">
                <span class="text-gray-300 text-base sm:text-sm">Маршрут:</span>
                <p class="m-0 text-lg sm:text-base">{{ orderItem.road }}</p>
            </div>
        </div>

        <div
            v-if="orderItem.cabins && orderItem.cabins.length > 0"
            class="py-6 px-5 border-b-2 border-gray-50 last:border-0"
        >
            <TotalItemCabin
                v-for="(cabinItem, cabinItemIdx) in orderItem.cabins"
                class="mb-7 last:mb-0"
                :key="`total-cabin-item-${cabinItemIdx}`"
                :cabin-item="cabinItem"
                :cruise-id="orderItem.cruiseID"
                :may-remove="false"
            />
        </div>

        <div
            v-if="orderItem.total"
            class="py-6 px-5 border-b-2 border-gray-50 last:border-0"
        >
            <p
                v-if="orderItem.total.tariffs"
                class="total-prices__item flex items-center justify-between m-0 mb-1"
            >
                <span class="total-prices__name">Круиз:</span>
                <span class="total-prices__value text-lg sm:text-lg">{{ formattedPrice(orderItem.total.tariffs) }}</span>
            </p>

            <p
                v-if="orderItem.total.excursions"
                class="total-prices__item flex items-center justify-between m-0 mb-1"
            >
                <span class="total-prices__name">Экскурсии:</span>
                <span class="total-prices__value text-lg sm:text-lg">{{ formattedPrice(orderItem.total.excursions) }}</span>
            </p>

            <p
                v-if="orderItem.total.discount"
                class="total-prices__item flex items-center justify-between m-0 mb-1"
            >
                <span class="total-prices__name">Скидка:</span>
                <span class="total-prices__value text-lg sm:text-lg">{{ formattedPrice(orderItem.total.discount) }}</span>
            </p>

            <p
                v-if="orderItem.total.total"
                class="total-prices__item flex items-center justify-between m-0 total-prices__item--last font-bold"
            >
                <span class="total-prices__name">Итого:</span>
                <span class="total-prices__value text-lg sm:text-lg">{{ formattedPrice(orderItem.total.total) }}</span>
            </p>
        </div>
    </div>
</template>
