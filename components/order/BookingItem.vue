<template>
    <div class="cart-item border-2 border-gray-50 rounded-sm">
        <div class="flex items-center p-5 bg-gray-50 2xl:block">
            <h3
                v-if="cartItem.name"
                class="cart-item__heading-item cart-item__title my-0 text-base leading-1-1 text-blue-900 2xl:mb-3 sm:leading-none"
            >
                <nuxt-link :to='getCruiseSlug'>
                    <span class="block w-full truncate">{{ cartItem.name }}</span>
                </nuxt-link>
            </h3>

            <div class="cart-item__heading-item cart-item__duration flex justify-start items-center text-sm 2xl:mb-2">
            ...
            <div
                v-if="cartItem.ship"
                class="cart-item__heading-item cart-item__ship flex items-center text-gray-300"
            >
                <svg
                ...
                <span class="flex-1-1 ml-3 font-medium truncate text-base">{{ cartItem.ship }}</span>
            </div>
        </div>

        <BookingCabinItem
            v-for="(cabinItem, cabinIdx) in cartItem.cabins"
            :key="cabinIdx"
...
import BookingCabinItem from "./BookingCabinItem";

export default {
    name: "BookingItem",
    components: { BookingCabinItem },
    mixins: [dayjsHelpers],
    props: {
        cartItem: {
            type: Object,
